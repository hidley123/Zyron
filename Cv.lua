...
function Library:_CreateUI()
    ...
    -- Player Info Container (foto e nome do player)
    ...
    RegisterThemeElement(playerAvatar, "BackgroundColor3", "Border")
    
    CreateCorner(playerAvatar, 14) -- Deixa circular
    
    -- Nome do player
    local playerNameLabel = CreateInstance("TextLabel", {
        Name = "PlayerName",
        Text = displayName ~= userName and displayName or userName,
        FontFace = f.Regular,
        TextColor3 = c.Text,
    ...
    -- Sistema de scroll touch para mobile na lista de tabs
    if IsMobileDevice() then
        ...
        tabsList.InputBegan:Connect(function(input)
        ...
        tabsList.InputChanged:Connect(function(input)
            if isDragging and input.UserInputType == Enum.UserInputType.Touch then
                local deltaY = input.Position.Y - lastTouchY
                local currentCanvasPosition = tabsList.CanvasPosition.Y
                local maxScroll = math.max(0, tabsList.CanvasSize.Y.Offset - tabsList.AbsoluteSize.Y)
                local newPosition = math.clamp(
                    currentCanvasPosition - deltaY,
                    0,
                    maxScroll
                )
                tabsList.CanvasPosition = Vector2.new(0, newPosition)
                scrollVelocity = deltaY
                lastTouchY = input.Position.Y
            end
        end)
        
        tabsList.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                isDragging = false
                -- Inércia do scroll
...
function Library:CreateTab(name)
    ...
    -- Sistema de scroll touch para mobile
    if IsMobileDevice() then
        local isDragging = false
        local lastTouchY = 0
        local scrollVelocity = 0
        ...
        tabContent.InputChanged:Connect(function(input)
            if isDragging and input.UserInputType == Enum.UserInputType.Touch then
                local deltaY = input.Position.Y - lastTouchY
                local currentCanvasPosition = tabContent.CanvasPosition.Y
                local maxScroll = math.max(0, tabContent.CanvasSize.Y.Offset - tabContent.AbsoluteSize.Y)
                local newPosition = math.clamp(
                    currentCanvasPosition - deltaY,
                    0,
                    maxScroll
                )
                tabContent.CanvasPosition = Vector2.new(0, newPosition)
                scrollVelocity = deltaY
                lastTouchY = input.Position.Y
            end
        end)
