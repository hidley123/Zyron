...
-- Variáveis para suporte universal
local AcrylicInstances = {}
...
-- Função para obter avatar do player
local function GetPlayerAvatar(userId)
    local success, result = pcall(function()
        return plr:GetUserThumbnailAsync(
            userId,
...
function Library.new(config)
    ...
    -- Registrar instância para suporte universal
    table.insert(AcrylicInstances, self)
...
function Library:_CreateUI()
    ...
    -- NOVO: Avatar e nome do player no canto superior esquerdo
    local playerInfoContainer = CreateInstance("Frame", {
        Name = "PlayerInfo",
        BackgroundColor3 = c.Secondary,
    ...
    RegisterThemeElement(playerInfoContainer, "BackgroundColor3", "Secondary")
    CreateCorner(playerInfoContainer, 14)
    local playerInfoStroke = CreateStroke(playerInfoContainer)
    RegisterThemeElement(playerInfoStroke, "Color", "Border")
    ...
    local avatarImage = CreateInstance("ImageLabel", {
        ...
        Image = GetPlayerAvatar(LocalPlayer.UserId),
        Parent = playerInfoContainer
    })
    RegisterThemeElement(avatarImage, "BackgroundColor3", "Secondary")
    ...
    RegisterThemeElement(playerNameLabel, "TextColor3", "Text")
    
    -- Ajustar tamanho do container baseado no texto
    ...
    playerInfoContainer.Size = UDim2.new(0, textSize.X + 40, 0, 28)
    ...
    local title = CreateInstance("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Position = UDim2.new(0, playerInfoContainer.Size.X.Offset + 15, 0, 0),
        Size = UDim2.new(1, -(playerInfoContainer.Size.X.Offset + 95), 1, 0),
        Parent = topBar
    })
    ...
    -- MELHORADO: ScrollingFrame com melhor suporte mobile
    local tabsList = CreateInstance("ScrollingFrame", {
        Name = "TabsList",
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ScrollBarThickness = IsMobileDevice() and 8 or 4,
...
function Library:Destroy()
    ...
    -- Remover da lista de instâncias universais
    for i, instance in ipairs(AcrylicInstances) do
        if instance == self then
            table.remove(AcrylicInstances, i)
            break
        end
    end
end

-- FUNÇÕES PARA SUPORTE UNIVERSAL
function Library.GetAllInstances()
    return AcrylicInstances
end

function Library.DestroyAll()
    for _, instance in ipairs(AcrylicInstances) do
        instance:Destroy()
    end
    AcrylicInstances = {}
end
